...
uint16_t adcToRC(int raw, int ch) {
    ...
    int v;
    if (raw <= mid) {
        v = map(raw, mn, mid, rcMin, rcMid);
    } else {
        v = map(raw, mid, mx, rcMid, RC_MAX);
    }
    v = constrain(v, rcMin, RC_MAX);
    int lowerRange = rcMid - rcMin;
    int upperRange = RC_MAX - rcMid;
    float norm;
    if (v >= rcMid) {
        norm = (upperRange > 0) ? ((float)(v - rcMid) / (float)upperRange) : 0.0f;
    } else {
        norm = (lowerRange > 0) ? ((float)(v - rcMid) / (float)lowerRange) : 0.0f;
    }
    norm = constrain(norm, -1.0f, 1.0f);
    norm = applyExpo(norm, expo[ch]);
    int mapped;
    if (norm >= 0.0f) {
        mapped = (int)(rcMid + norm * (float)upperRange);
    } else {
        mapped = (int)(rcMid + norm * (float)lowerRange);
    }
    mapped = constrain(mapped, rcMin, RC_MAX);
    if (reversed[ch]) mapped = rcMin + RC_MAX - mapped;
    mapped += trim[ch];
    mapped = constrain(mapped, rcMin, RC_MAX);
...
void sendBle() {
    ...
    if (connected != bleWasConnected) {
        ...
        Serial.println(connected ? "BLE sim connected" : "BLE sim disconnected");
    }
    if (!connected) return;
...
void printChannels() {
    ...
    Serial.print("HZ    "); Serial.println(controlHz);
    Serial.print("BLE   "); Serial.println(bleGamepad.isConnected() ? "connected" : "advertising");
    Serial.println("================");
}
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .ble-pill{display:inline-block;padding:3px 10px;border-radius:12px;font-size:0.78rem;font-weight:bold;margin-left:10px}
  .ble-on{background:#00695c;color:#fff}
  .ble-off{background:#333;color:#888}
  .hz-row{display:flex;align-items:center;gap:12px}
  .hz-row input[type=range]{flex:1}
  .hz-num{color:#4fc3f7;font-size:1.1rem;font-weight:bold;min-width:60px;text-align:right}
</style>
</head>
<body>
<div class="toast" id="toast">Saved</div>
<h1>Transmitter Config <span class="ble-pill ble-off" id="blePill">BLE --</span></h1>
...
<div class="hz-row">
  <span>10 Hz</span>
  <input type="range" id="hzSlider" min="10" max="200" value="50" oninput="onHzSlide(this.value)">
  <span>200 Hz</span>
  <span class="hz-num"><span id="hzVal">50</span> Hz</span>
</div>
</div>
...
function onHzSlide(v){
  document.getElementById('hzVal').textContent=v;
  clearTimeout(hzTimeout);
  hzTimeout=setTimeout(function(){
    fetch('/set_hz?val='+v).then(function(r){if(r.ok)showToast();});
  },400);
}
...
function pollData(){
  fetch('/data').then(function(r){return r.json();}).then(function(d){
    ...
    if(sl && document.activeElement!==sl){
      sl.value=d.hz;
      document.getElementById('hzVal').textContent=d.hz;
    }
    var pill=document.getElementById('blePill');
    if(d.ble){
      pill.textContent='BLE connected';
      pill.className='ble-pill ble-on';
    } else {
      pill.textContent='BLE advertising';
      pill.className='ble-pill ble-off';
    }
  }).catch(function(){});
...
void handleSetHz() {
    if (!server.hasArg("val")) { server.send(400, "text/plain", "Bad"); return; }
    int v = server.arg("val").toInt();
    controlHz = (uint16_t)constrain(v, 10, 200);
    packetInterval = 1000 / controlHz;
    eepromSaveHz();
...
void handleData() {
    ...
    json += controlHz;
    json += ",\"ble\":";
    json += bleGamepad.isConnected() ? "true" : "false";
    json += "}";
...
void handleSave() {
    ...
    if (body.indexOf("\"type\":\"cal\"") >= 0) {
        for (int i = 0; i < 4; i++) {
            String key; int pos;
            key = "\"calMin\":["; pos = body.indexOf(key);
            if (pos >= 0) {
                String sub = body.substring(pos + key.length());
                int vals[4], cnt=0, start=0;
                while (cnt < 4) {
                    int comma=sub.indexOf(',',start), bracket=sub.indexOf(']',start);
                    int end=(comma>=0&&comma<bracket)?comma:bracket;
                    vals[cnt++]=sub.substring(start,end).toInt();
                    if(end==bracket) break; start=end+1;
                }
                for(int j=0;j<4;j++) calMin[j]=vals[j];
            }
            key = "\"calMid\":["; pos = body.indexOf(key);
            if (pos >= 0) {
                String sub = body.substring(pos + key.length());
                int vals[4], cnt=0, start=0;
                while (cnt < 4) {
            ...
            key = "\"calMax\":["; pos = body.indexOf(key);
            if (pos >= 0) {
                String sub = body.substring(pos + key.length());
                int vals[4], cnt=0, start=0;
                while (cnt < 4) {
        ...
        eepromSave(); server.send(200, "text/plain", "OK"); return;
    }

    if (body.indexOf("\"type\":\"reversed\"") >= 0) {
        String key="\"reversed\":["; int pos=body.indexOf(key);
        if (pos >= 0) {
            String sub=body.substring(pos+key.length());
            int cnt=0,start=0;
            while(cnt<10){
                int comma=sub.indexOf(',',start),bracket=sub.indexOf(']',start);
                int end=(comma>=0&&comma<bracket)?comma:bracket;
                String tok=sub.substring(start,end); tok.trim();
    ...
    if (body.indexOf("\"type\":\"trim\"") >= 0) {
        String key="\"trim\":["; int pos=body.indexOf(key);
        if (pos >= 0) {
            String sub=body.substring(pos+key.length());
            int cnt=0,start=0;
            while(cnt<10){
    ...
    if (body.indexOf("\"type\":\"expo\"") >= 0) {
        String key="\"expo\":["; int pos=body.indexOf(key);
        if (pos >= 0) {
            String sub=body.substring(pos+key.length());
            int cnt=0,start=0;
            while(cnt<4){
...
void setup() {
    ...
    for (int i = 0; i < 4; i++) {
        float r = (float)readAvg(bootPins[i]);
        smoothedADC[i] = r;
        calMid[i] = (int)r;
    }
...
void loop() {
    ...
    if (millis() - lastTx < packetInterval) return;
    lastTx = millis();
